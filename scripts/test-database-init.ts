...
async function testTableSchemas() {
  ...
  const missingBookColumns = expectedBookColumns.filter(
    (col) => !bookColumnNames.includes(col),
  );
  ...
  const missingUserColumns = expectedUserColumns.filter(
  ...
  if (missingSessionColumns.length === 0) {
    ...
    console.error(
      "❌ reading_sessions missing columns:",
  ...
  const expectedProgressColumns = [
    "id",
    "weeks_passed",
    "target_reading_minutes",
    "achived_reading_minutes",
    "date_created",
  ];

  const progressColumnNames = progressColumns.map((col: any) => col.name);
  const missingProgressColumns = expectedProgressColumns.filter(
    (col) => !progressColumnNames.includes(col),
  );
  ...
  if (missingProgressColumns.length === 0) {
    ...
    console.error(
      "❌ weekly_progress missing columns:",
      missingProgressColumns,
    );
  }
}
...
async function testSampleData() {
  try {
    ...
    const bookCount = await queryAll(
      "SELECT COUNT(*) as count FROM enhanced_books",
    );
    const userCount = await queryAll(
      "SELECT COUNT(*) as count FROM user_preferences",
    );
    const sessionCount = await queryAll(
      "SELECT COUNT(*) as count FROM reading_sessions",
    );
    const progressCount = await queryAll(
      "SELECT COUNT(*) as count FROM weekly_progress",
    );
