...
export class NotificationService {
  ...
  async syncPermissionsWithDatabase(): Promise<void> {
    try {
      ...
      if (
        !hasSystemPermission &&
        (prefs.notifications_enabled || prefs.daily_reminder_enabled)
      ) {
        console.log(
          "🔄 System permissions denied, updating database to reflect this",
        );
  ...
  async getNotificationPreferences(): Promise<NotificationPreferences | null> {
    try {
      ...
      if (!prefs) {
        console.log(
          "🔔 No notification preferences found, creating defaults...",
        );
  ...
  async updateNotificationPreferences(
    ...
    try {
      ...
      if (
        updates.notifications_enabled === false ||
        updates.daily_reminder_enabled === false
      ) {
        await this.cancelScheduledNotification();
      }
  ...
  private async createDefaultNotificationPreferences(): Promise<void> {
    try {
      ...
      if (isFirstInstall) {
        console.log(
          "🔔 First app installation detected, requesting notification permissions...",
        );
        ...
        try {
          ...
          console.error(
            "⚠️ Error requesting permissions on first install:",
            permissionError,
          );
      ...
      if (isFirstInstall) {
        console.log(
          "🔔 Created default notification preferences for first install:",
          {
  ...
  async isDailyGoalMet(): Promise<boolean> {
    try {
      ...
      if (userPrefs?.current_reading_rate_minutes_per_day) {
        ...
        try {
          const basicUserPrefs = await queryFirst<{
      ...
      try {
        ...
        console.log(
          "📝 reading_sessions table not found, assuming no reading today",
        );
        return false;
      }
  ...
  async checkAndScheduleNotification(): Promise<void> {
    try {
      ...
      if (!isLoggedIn) {
        console.log(
          "👤 No user logged in, skipping notification check and scheduling",
        );
        return;
      }
  ...
  private async trackAppOpened(): Promise<void> {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Failed to recreate app usage tracking:",
          recreateError,
        );
      }
    }
  ...
  private async trackAppClosed(): Promise<void> {
    try {
      ...
      try {
        ...
        console.error(
          "❌ Failed to recreate app usage tracking for close event:",
          recreateError,
        );
      }
    }
  ...
  async getNotificationStatus(): Promise<{
    ...
    try {
      const [enabled, permissions, lastOpened, scheduled, prefs] =
        await Promise.all([
  ...
  async reset(): Promise<void> {
    try {
      ...
      try {
        ...
        if (
          !integrity.notification_preferences_exists ||
          !integrity.notification_preferences_has_defaults
        ) {
          console.log(
            "🔧 Notification tables missing or incomplete, attempting repair...",
          );
        ...
        if (prefs) {
          console.log(
            "🔔 Notification service reset successfully and preferences restored",
          );
        } else {
          console.log(
            "⚠️ Notification service reset but could not restore preferences",
          );
        }
      } catch (dbError) {
        console.error(
          "⚠️ Database integrity check failed during reset:",
          dbError,
        );
        ...
        try {
          await this.createDefaultNotificationPreferences();
          console.log(
            "🔔 Created default notification preferences as fallback",
          );
        } catch (fallbackError) {
          console.error(
            "❌ Failed to create default notification preferences as fallback:",
            fallbackError,
          );
        }
      }
